...
BASE_URL = f"http://openapi.seoul.go.kr:8088/{API_KEY}/json/tbLnOpendataRentV"
...
for y in years:
    for start in range(1, END_LIMIT, STEP):
        ...
        if res.status_code != 200:
            ...
            break
        data = res.json().get("tbLnOpendataRentV", {}).get("row", [])
        if not data:
...
df_seongsu = df[mask]
df_seongsu.to_csv("rent_seongsu.csv", index=False, encoding="utf-8-sig")
